...
export function TeacherAssignmentsList({ userId }: TeacherAssignmentsListProps) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      setShowForm(false)
      setFormData({ title: '', description: '', dueDate: '', course: '', class: '' })
      toast({
        title: "Başarılı",
  ...
  return (
    <div>
      <Button className="mb-6 cyberpunk-button" onClick={() => setShowForm(!showForm)}>
        <Plus className="mr-2" />
      ...
      {showForm && (
        <form onSubmit={handleSubmit} className="mb-6 space-y-4">
          ...
          <Button type="submit" className="cyberpunk-button">Ödev Ekle</Button>
        </form>
      )}
