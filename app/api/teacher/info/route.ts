...
export async function GET(req: NextRequest) {
  try {
    ...
    const classes = Array.from(new Set(courses.map(course => course.class.name)))

    const teacherInfo = {
      name: teacher.name,
      email: teacher.email,
      branches: teacher.branches.map((branch: { name: string }) => branch.name),
      classes: classes,
    }
    ...
    return NextResponse.json({ error: 'Öğretmen bilgileri alınırken bir hata oluştu' }, { status: 500 })
  }
}
