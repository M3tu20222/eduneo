...
export async function POST(req: NextRequest) {
  try {
    ...
    const { title, description, dueDate, course, class: classId, teacher } = await req.json()
...
export async function GET(req: NextRequest) {
  try {
    ...
    return NextResponse.json({ error: 'Ödevler alınırken bir hata oluştu' }, { status: 500 })
  }
}
