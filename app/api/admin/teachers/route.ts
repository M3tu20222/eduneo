...
export async function GET(req: NextRequest) {
  try {
    ...
    const teachersWithClassInfo = await Promise.all(teachers.map(async (teacher) => {
    ...
    return NextResponse.json({ error: 'Öğretmenler alınırken bir hata oluştu' }, { status: 500 })
  }
}
...
export async function POST(req: NextRequest) {
  try {
    ...
    const branchIds = await Branch.find({ _id: { $in: branches } }).distinct('_id')
    const classIds = await Class.find({ _id: { $in: classes } }).distinct('_id')

    if (branchIds.length !== branches.length || classIds.length !== classes.length) {
      return NextResponse.json({ error: 'Geçersiz branş veya sınıf ID\'si' }, { status: 400 })
    }
